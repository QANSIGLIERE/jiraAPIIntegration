...
class JIRA_API {
    ...
    async get_all_users(maxResults = 50) {
        return await get(this.url, `/rest/api/3/users/search?maxResults=${maxResults}`, this.headers);
    }

    async get_all_users_by_default(maxResults = 50) {
        return await get(this.url, `/rest/api/3/users?maxResults=${maxResults}`, this.headers);
    }
}
